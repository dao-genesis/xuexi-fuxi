...
4. 扭矩 $M=F\cdot r=12.57\times0.1=1.26\ \mathrm{N\cdot m}$。

$$\boxed{M=\mu\frac{\omega r}{\delta}\,(2\pi r L)\,r\approx1.26\ \mathrm{N\cdot m}}$$
...
**题**：两水池由薄壁圆孔相连，孔口面积 $A=30\ \text{cm}^2$，上下游水位差 $\Delta H=1.6\ \text{m}$，流量系数 $\mu=0.62$。求过孔流量 $Q$。

**解**：

1. 淹没出流以上下游**水位差** $\Delta H$ 为作用水头：$\sqrt{2g\Delta H}=\sqrt{2\times9.8\times1.6}=\sqrt{31.36}=5.6\ \text{m/s}$。
2. $A=30\ \text{cm}^2=3\times10^{-3}\ \text{m}^2$。
3. $Q=\mu A\sqrt{2g\Delta H}=0.62\times3\times10^{-3}\times5.6=1.04\times10^{-2}\ \text{m}^3/\text{s}$。

$$\boxed{Q=\mu A\sqrt{2g\Delta H}\approx10.4\ \text{L/s}}$$

> 易错：淹没出流用**水位差** $\Delta H$，不是某一侧的淹深；且淹没出流的 $\mu$ 与自由出流近似相等。

### 例 6-4（圆柱形外管嘴 vs 孔口 · 增流对比）

**题**：同一水箱、同一孔径 $d=50\ \text{mm}$、作用水头 $H=3\ \text{m}$。比较薄壁孔口（$\mu=0.62$）与外接圆柱形管嘴（$\mu_n=0.82$）的出流量。

**解**：

1. 面积 $A=\dfrac{\pi}{4}d^2=\dfrac{\pi}{4}(0.05)^2=1.963\times10^{-3}\ \text{m}^2$；$\sqrt{2gH}=\sqrt{2\times9.8\times3}=7.67\ \text{m/s}$。
2. 孔口：$Q=\mu A\sqrt{2gH}=0.62\times1.963\times10^{-3}\times7.67=9.33\times10^{-3}\ \text{m}^3/\text{s}$。
3. 管嘴：$Q_n=\mu_n A\sqrt{2gH}=0.82\times1.963\times10^{-3}\times7.67=1.234\times10^{-2}\ \text{m}^3/\text{s}$。
4. 增流比 $Q_n/Q=\mu_n/\mu=0.82/0.62=1.32$。

$$\boxed{Q\approx9.3\ \text{L/s},\quad Q_n\approx12.3\ \text{L/s},\quad Q_n/Q\approx1.32}$$

> 物理原因：圆柱形外管嘴收缩断面处形成真空（$h_v\approx0.75H$），等效增大作用水头，故流量比同径孔口大约三成。

### 例 6-5（并联管道 · 流量分配）

**题**：两节点间并联两条管，水头损失关系 $h_f=SQ^2$（$S$ 为阻抗）。已知 $S_1=200\ \text{s}^2/\text{m}^5$、$S_2=800\ \text{s}^2/\text{m}^5$，总流量 $Q=0.06\ \text{m}^3/\text{s}$。求各支路流量。

**解**：

1. 并联各支路**水头损失相等**：$S_1Q_1^2=S_2Q_2^2\Rightarrow \dfrac{Q_1}{Q_2}=\sqrt{\dfrac{S_2}{S_1}}=\sqrt{\dfrac{800}{200}}=2$。
2. 又 $Q_1+Q_2=0.06$，代入 $Q_1=2Q_2$：$3Q_2=0.06\Rightarrow Q_2=0.02,\ Q_1=0.04\ \text{m}^3/\text{s}$。

$$\boxed{Q_1=40\ \text{L/s},\quad Q_2=20\ \text{L/s}}$$

> 规律：并联管段阻抗小者分配流量大；阻抗与流量平方之积（即水头损失）各支路相等。
...
**题**：梯形渠底宽 $b=3\ \text{m}$、水深 $h=1.2\ \text{m}$、边坡系数 $m=1.5$、底坡 $i=0.0006$、糙率 $n=0.025$。求流量 $Q$。

**解**：

1. 过水面积 $A=(b+mh)h=(3+1.5\times1.2)\times1.2=(3+1.8)\times1.2=5.76\ \text{m}^2$。
2. 湿周 $\chi=b+2h\sqrt{1+m^2}=3+2\times1.2\times\sqrt{1+1.5^2}=3+2.4\times1.803=7.33\ \text{m}$。
3. 水力半径 $R=A/\chi=5.76/7.33=0.786\ \text{m}$。
4. 谢才系数 $C=\dfrac{1}{n}R^{1/6}=\dfrac{1}{0.025}\times0.786^{1/6}=40\times0.961=38.4\ \text{m}^{1/2}/\text{s}$。
5. $v=C\sqrt{Ri}=38.4\times\sqrt{0.786\times0.0006}=38.4\times\sqrt{4.72\times10^{-4}}=38.4\times0.0217=0.834\ \text{m/s}$。
6. $Q=vA=0.834\times5.76=4.80\ \text{m}^3/\text{s}$。

$$\boxed{Q=AC\sqrt{Ri}\approx4.8\ \text{m}^3/\text{s}}$$

### 例 7-4（矩形渠 · 临界水深与流态判别）

**题**：矩形渠宽 $b=4\ \text{m}$，通过流量 $Q=8\ \text{m}^3/\text{s}$，实际水深 $h=1.5\ \text{m}$（取动能修正系数 $\alpha=1$）。求临界水深 $h_c$ 并判别流态。

**解**：

1. 单宽流量 $q=Q/b=8/4=2\ \text{m}^2/\text{s}$。
2. 临界水深 $h_c=\sqrt[3]{\dfrac{\alpha q^2}{g}}=\sqrt[3]{\dfrac{1\times2^2}{9.8}}=\sqrt[3]{0.408}=0.742\ \text{m}$。
3. 实际水深 $h=1.5\ \text{m}>h_c=0.742\ \text{m}$，故为**缓流**。
4. 校核：$v=Q/(bh)=8/(4\times1.5)=1.33\ \text{m/s}$，$Fr=\dfrac{v}{\sqrt{gh}}=\dfrac{1.33}{\sqrt{9.8\times1.5}}=\dfrac{1.33}{3.83}=0.347<1$，与缓流一致。

$$\boxed{h_c\approx0.74\ \text{m},\quad h>h_c\ (Fr\approx0.35<1)\Rightarrow\text{缓流}}$$

### 例 7-5（水力最优梯形断面设计）

**题**：梯形土渠边坡系数 $m=1.0$，按水力最优断面设计，已知设计水深 $h=1.0\ \text{m}$。求最优底宽 $b$ 与水力半径 $R$。

**解**：

1. 水力最优梯形宽深比 $\beta=\dfrac{b}{h}=2\left(\sqrt{1+m^2}-m\right)=2(\sqrt{2}-1)=2\times0.414=0.828$。
2. 最优底宽 $b=\beta h=0.828\times1.0=0.828\ \text{m}$。
3. 水力最优断面的水力半径恒为 $R=h/2=0.5\ \text{m}$（梯形最优断面的重要性质）。

$$\boxed{b=2(\sqrt{1+m^2}-m)\,h\approx0.83\ \text{m},\quad R=h/2=0.5\ \text{m}}$$

> 性质：任意边坡的水力最优梯形断面，其水力半径都等于水深之半 $R=h/2$，可作快速校核。
